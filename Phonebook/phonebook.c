...
void namefun(){
	...
	double phone;
	char gender[8];
	fptr=fopen("rwitam.txt","ab+");//ab+ gives us the ability of writing the function and add the second data in the existing one...
	if(fptr==NULL){
...
void modifyfun(){
	...
	fclose(fptr);
	fclose(fptr1);
	fptr=fopen("rwitam.txt","w");
	fclose(fptr);
	fptr=fopen("rwitam.txt","a");
...
void exitfun(){
	...
	gotoxy(31,6);
	printf("\xDB rwitam ray.");
	gotoxy(31,8);
	printf("\xDB sambhabi das.");
	gotoxy(31,10);
	printf("\xDB sohel datta.");
	gotoxy(31,12);
	printf("\xDB srijita dutta.");
}
void password(void){
	char passwords[20]={"rwitam\\"};
	gotoxy(22,2);
	int j;
	int z;
